...
CREATE OR REPLACE FUNCTION ESTADISTICA_ESTADOS(estado VARCHAR(80)) RETURNS TABLE(MUNICIPIO VARCHAR(80), AVGANSIEDADTOTAL FLOAT(30)) AS $$
DECLARE

BEGIN
	 RETURN QUERY
	 SELECT LM.NOMBRE, AVG(D.ANSIEDADTOTAL)
	 FROM DIAGNOSTICO D, PACIENTE P, LUGAR LE, LUGAR LM, LUGAR LP
	 WHERE D.FK_PACIENTE = P.CI AND P.FK_LUGAR = LP.CODIGO AND LP.FK_LUGAR = LM.CODIGO AND LM.FK_LUGAR = LE.CODIGO AND LE.NOMBRE = estado
	 GROUP BY 1;
END;
$$ LANGUAGE plpgsql;     

CREATE OR REPLACE FUNCTION ESTADISTICA_EDAD() RETURNS TABLE(EDAD INTEGER, AVGANSIEDADTOTAL FLOAT(30)) AS $$
DECLARE

BEGIN
	 RETURN QUERY
	 SELECT P.EDAD, COUNT(D.*)
...
CREATE OR REPLACE FUNCTION ESTADISTICA_CARRERA() RETURNS TABLE(EDAD VARCHAR(35), AVGANSIEDADTOTAL FLOAT(30)) AS $$
DECLARE
