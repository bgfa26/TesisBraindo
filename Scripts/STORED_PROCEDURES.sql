...
CREATE OR REPLACE FUNCTION DIAGNOSTICO_CONSULTAR_IDPSYCHO(PSYCHO integer) RETURNS TABLE(CODIGO integer, FECHA date, SENTIMIENTOS varchar(100), EMOCIONES varchar(100), RESPUESTA varchar(120), RESPUESTARED varchar(200), CEDULA_PACIENTE integer, NOMBRE_PACIENTE varchar(50), APELLIDO_PACIENTE varchar(50), CEDULA_PSICO integer, NOMBRE_PSICO varchar(50), APELLIDO_PSICO varchar(50)) AS $$
DECLARE

BEGIN
	 RETURN QUERY
	 SELECT D.CODIGO, D.FECHA, D.SENTIMIENTOS, D.EMOCIONES, D.RESPUESTA, D.RESPUESTARED, P.CI, P.PRIMERNOMBRE, P.PRIMERAPELLIDO, PS.CEDULA, PS.PRIMERNOMBRE, PS.PRIMERAPELLIDO
	 FROM DIAGNOSTICO D, PACIENTE P, PSICOLOGO PS
	 WHERE D.FK_PSICOLOGO = PSYCHO AND PS.CEDULA = D.FK_PSICOLOGO;
END;
$$ LANGUAGE plpgsql;
