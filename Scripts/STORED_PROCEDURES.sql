...
CREATE OR REPLACE FUNCTION CITA_REGISTRAR(date, time, varchar(200), integer, integer) RETURNS integer AS $$
DECLARE
 RESULT integer;

BEGIN

	INSERT INTO CITA (FECHA, HORA, MOTIVO, FK_PACIENTE, FK_PSICOLOGO) VALUES ($1, $2, $3);
	RESULT := 201;
	RETURN RESULT;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION CITA_CONSULTAR(integer, integer) RETURNS TABLE(CODIGO integer, FECHA date, HORA time, MOTIVO varchar(200), FK_PACIENTE integer, FK_PSICOLOGO integer) AS $$
DECLARE
